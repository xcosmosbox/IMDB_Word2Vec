...
def train_autoencoder(
    tabular_features_path: Path,
    max_rows: Optional[int] = None,
) -> Path:
    """训练自编码器以融合高维离散特征，输出稠密表示。

    Args:
        tabular_features_path: 表格特征 CSV 路径（来自 feature_engineering）。
        max_rows: 可选的行数上限，用于小样本验证。
    """
    df = pd.read_csv(tabular_features_path)
    
    # 移除 tconst 列（如果存在），只保留特征列
    if "tconst" in df.columns:
        tconst_col = df["tconst"]  # 保存以便后续使用
